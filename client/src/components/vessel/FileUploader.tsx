...
function FileUploader(props: FileUploaderType) {
	...
	const getHeaderTitle = () => {
		if (isPaused) {
			...
			return `${remainingFiles} files remaining`;
		}
		...
		<Dialog onOpenChange={handleResetUploader}>
			...
			>
				<div className=" w-full relative">
					...
					>
						<DialogTitle className="text-md md:text-lg font-medium text-gray-500 font-inter text-left">
							{getHeaderTitle()}
							{allFilesCompleted && (
					...
					<div className={`${!isMinimized && 'p-6'} `}>
						...
						{showGrid && (
							<motion.p
								initial={{ opacity: 0 }}
								animate={{ opacity: 1 }}
								className="text-xs text-gray-400 text-center leading-relaxed md:px-5 mt-3"
							>
